...
class GLViewWidgetPos(pg.opengl.GLViewWidget):
    def __init__(self, parent=None, name=None, last_vb=[None]):
        ''' GLViewWidget with minor additional functionality .'''
        super().__init__(parent)
        self.parent = parent
        assert type(name) == str
        self.name = name
        self.last_vb = last_vb  # this should be a list so that the object is mutable
    ...
    def __repr__(self):
        ...
        return f'Instance {self.name} of GLViewWidgetPos Class'

    def mousePressEvent(self, ev):
        super(GLViewWidgetPos, self).mousePressEvent(ev)
        self.last_vb[0] = self  # set last_viewbox to self
        ev.accept()  # If you do not accept the event, then no move/release events will be received.
...
class ShortcutsDialog(QtWidgets.QDialog):
    def __init__(self, parent):
        ...
        try:
            with open(BasePath + '/../../Content/shortcuts.txt', 'r') as f:
                st = f.readlines()
...
class StaticDataDialog(QtWidgets.QDialog):
    def __init__(self, parent):
        super().__init__(parent)
        self.threshold_params = {'binarise': False, 'threshold': 0}
        self.setSizePolicy(QtWidgets.QSizePolicy.Expanding, QtWidgets.QSizePolicy.Expanding)
        ...
        self.load_button.clicked.connect(lambda: parent.load_static(threshold_params=self.threshold_params))
        ...
        self.threshold_cbx = QtWidgets.QCheckBox('Binarise data with absolute threshold', self)
        self.threshold_cbx.stateChanged.connect(self.toggle_threshold)
        self.threshold_spbx = QtWidgets.QDoubleSpinBox()
        self.threshold_spbx.setValue(0.05)
        self.threshold_spbx.setRange(-10, 10)
        self.threshold_spbx.setDecimals(3)
        self.threshold_spbx.setSingleStep(0.01)
        self.threshold_spbx.setFixedWidth(80)
        self.threshold_spbx.setEnabled(False)  # default if cbx is unchecked
        self.threshold_spbx.valueChanged.connect(self.change_threshold)
        self.threshold_layout.addWidget(self.threshold_cbx)
        self.threshold_layout.addWidget(self.threshold_spbx)


        self.v_layout.addLayout(self.load_layout)
        self.v_layout.addSpacerItem(QtWidgets.QSpacerItem(40, 10, QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Minimum))
        self.v_layout.addLayout(self.threshold_layout)
        self.v_layout.addSpacerItem(QtWidgets.QSpacerItem(40, 10, QtGui.QSizePolicy.Expanding, QtGui.QSizePolicy.Minimum))
    ...
    def toggle_threshold(self):
        if self.threshold_cbx.isChecked():
            self.threshold_spbx.setEnabled(True)
            self.threshold_params['binarise'] = True
            self.change_threshold()  # for initialisation
        else:
            self.threshold_spbx.setEnabled(False)
            self.threshold_params['binarise'] = False

    def change_threshold(self):
        self.threshold_params['threshold'] = self.threshold_spbx.value()
...
class LogModel(QtCore.QAbstractTableModel):
    ...
    def data(self, index, role):
        ...
        if not index.isValid():
            ...
            return QtCore.QVariant()
        # Return the data at index.row() and index.column()
...
class ColoringDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent,  datasets=(), data=None, version=None):  #version= 'Region' or 'Cluster'
        ...
        self.deselect_button = QtWidgets.QPushButton("Deselect all")
        ...
        self.h_layout.addWidget(self.deselect_button)
    ...
    def create_table(self):
        '''Create the table and store in self.cluster_table'''
        self.cluster_model = ColorModel(self.data, self.version)
        self.cluster_table = QtWidgets.QTableView(self)
        self.cluster_table.setModel(self.cluster_model)
        self.cluster_table.setItemDelegate(TableDelegate(self.cluster_table, datasets=self.datasets))
        header = self.cluster_table.horizontalHeader()
        header.setSectionResizeMode(0, QtWidgets.QHeaderView.Stretch)

    def deselect_all(self):
        '''deselect all entries.
        #TODO: refresh table (currently needs a mouse click to show result)'''
        for ii in range(len(self.data)):
            self.data[ii]['Selected'] = 0
        self.create_table()
